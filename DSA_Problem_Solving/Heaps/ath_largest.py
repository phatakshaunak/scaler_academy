...
class Solution:
    ...
    def solve(self, A, B):
        ...
        ans = []
        # Fill -1 for subarrays smaller than size A
        for i in range(A-1):
            ans.append(-1)
        
        # Finding the Ath largest element. If we do a min heap of A elements, the min value should be the Ath largest value
        
        # Min heap of first A elements
        tmp = []
        for i in range(A):
            tmp.append(B[i])
