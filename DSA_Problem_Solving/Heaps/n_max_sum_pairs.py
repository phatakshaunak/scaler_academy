...
class Solution:
    ...
    # Start from the right of sorted arrays ; Use max heap to get best sum pair N times
    # Insert last element sum first, then two options each time (i-1, j) and (i, j-1).
    # Also ensure same index pairs don't get inserted into the heap again (use a set)
    # Pop max element of the heap N times to get N max sums